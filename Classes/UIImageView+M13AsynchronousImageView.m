...
{
    [self loadImageAtURL:url target:nil checkCache:YES completion:nil];
}



- (void)loadImageAtURL:(NSURL *)url target:(id)target checkCache:(BOOL)checkCache completion:(M13AsynchronousImageLoaderCompletionBlock)completion
{
...
- (void)startLoading
{
    ...
    else
    {
        ...
        NSURLRequest *request = [NSURLRequest requestWithURL:_fileURL cachePolicy:NSURLRequestReloadIgnoringLocalAndRemoteCacheData timeoutInterval:_timeoutInterval];
        // Create a connection
...
{
    [[M13AsynchronousImageLoader defaultLoader] loadImageAtURL:url target:self checkCache:YES completion:^(BOOL success, M13AsynchronousImageLoaderImageLoadedLocation location, UIImage *image, NSURL *url, id target) {
...
- (void)loadImageFromURL:(NSURL *)url checkCache:(BOOL)checkCache completion:(M13AsynchronousImageLoaderCompletionBlock)completion
{
    [[M13AsynchronousImageLoader defaultLoader] loadImageAtURL:url target:self checkCache:checkCache completion:^(BOOL success, M13AsynchronousImageLoaderImageLoadedLocation location, UIImage *image, NSURL *url, id target) {
         // Set the image if loaded
         if (success)
         {
             self.image = image;
